...
function FruitsPage() {
  ...
  const fetchFruits = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:8000/api/category_products.php?farmer_id=${farmerId}&category=fruits`);
      ...
      console.log('Fetch fruits response:', data);
      
      if (data.success) {
        setFruits(data.data || []);
      } else {
        console.error('Failed to fetch fruits:', data.message);
        setFruits([]);
      }
    } catch (error) {
  ...
  const handleEditProduct = async (productId, updatedData) => {
    try {
      const response = await fetch(`http://localhost:8000/api/category_products.php?product_id=${productId}&category=fruits&farmer_id=${farmerId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        ...
        <div className="products-grid">
          {loading ? (
            ...
            <div className="no-products">
              <p>No fruits found. Add your first fruit product!</p>
            </div>
          ) : (
            fruits.map(product => (
              <div key={product.id} className="product-card">
                <div className="product-image">
                  <img 
                    src={product.image_url || '/src/assets/images/fruits.jpg'} 
                    alt={product.name}
                    onError={(e) => {
                      e.target.src = '/src/assets/images/fruits.jpg';
                    }}
                  />
                </div>
