...
function DairyPage() {
  ...
  // Fetch dairy products from backend
  const fetchDairyProducts = async () => {
    try {
      setLoading(true);
      
      const response = await fetch(`http://localhost:8000/api/category_products.php?farmer_id=${farmerId}&category=dairy`);
      ...
      console.log('Fetch dairy products response:', data);
      
      if (data.success) {
        setDairyProducts(data.data || []);
      } else {
        console.error('Failed to fetch dairy products:', data.message);
        setDairyProducts([]);
      }
    } catch (error) {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setNewProduct({ name: '', description: '', price: '', stock: '', unit: 'litre' });
        fetchDairyProducts(); // Refresh the list
    ...
    <div className="category-page">
      ...
      <div className="category-container">
        <div className="category-header">
          ...
          <h1>🥛 Dairy Products Management</h1>
          <p>Manage your dairy inventory</p>
        </div>
        ...
        <div className="products-grid">
          {loading ? (
            ...
            <div className="no-products">
              <p>No dairy products found. Add your first dairy product!</p>
            </div>
          ) : (
            dairyProducts.map(product => (
              <div key={product.id} className="product-card">
                <div className="product-image">
                  <img 
                    src={product.image_url || '/src/assets/images/dairy.jpg'} 
                    alt={product.name}
                    onError={(e) => {
                      e.target.src = '/src/assets/images/dairy.jpg';
                    }}
                  />
                </div>
